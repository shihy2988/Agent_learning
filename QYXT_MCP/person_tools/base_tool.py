...
class Base_tool:
    ...
    def format_duration_seconds(self,start_str):
        ...
        return  self.seconds_to_time_str(diff)
    ...
    def classify_segments_by_inout(self, segments, inout_records, tolerance_sec=120):
        ...
        if not inout_records:
            ...
            return [{
                "入井时间": first_seg.get("进入时间"),
                "出井时间": None,
                "入井地点":first_seg.get("主站位置"),
                "出井地点": None,
                "入井时长": self.format_duration_seconds(first_seg.get("进入时间")),
                "轨迹个数": len(segments),
                "入井轨迹": segments
            }]
        ...
        segments_sorted = sorted(segments, key=lambda s: s.get("进入时间", ""))
        ...
        for idx_r, record in enumerate(inout_records):
            ...
            if not in_time_raw or not in_place:
                if segments_sorted and len(result) == 0:
                    ...
                    result.append({
                        ...
                        "出井地点": None,
                        "入井时长": self.format_duration_seconds(first_seg.get("进入时间")),
                        "轨迹个数": len(segments_sorted),
                        "入井轨迹": segments_sorted
                    })
                continue
            ...
            for idx, seg in enumerate(segment_objs):
                try:
                    seg_start = self.parse_time(seg["进入时间"][:19].replace('+08:00', ''))
                    seg_end = self.parse_time(seg["离开时间"][:19].replace('+08:00', ''))
                except Exception:
                    continue
            ...
            if grouped_indices:
                ...
                if n_group:
                    # 修改最后一个seg的结束时间/主站点
                    group_segs[-1]["离开时间"] = out_time_record
                    if record.get("OutWellPlace"):
                        group_segs[-1]["主站位置"] = record.get("OutWellPlace")
                # 如果不是第一组，且上一组存在，修改当前组第一个seg的开始时间/主站点
                if idx_r > 0 and n_group:
                    prev_record = inout_records[idx_r]
                    group_segs[0]["进入时间"] = in_time_raw
                    group_segs[0]["主站位置"] = in_place

                result.append({
                    "入井时间": in_time_raw,
                    "出井时间": out_time_record,
                    "入井地点":in_place,
        ...
        if ungrouped_segments:
            ...
            result.append({
                ...
                "出井地点": None,
                "入井时长": self.format_duration_seconds(first_seg.get("进入时间")),
                "轨迹个数": len(ungrouped_segments),
                "入井轨迹": ungrouped_segments
            })
    ...
    def get_persons_by_filters(
        ...
        flagged AS (
            SELECT
                *,
                -- 修正点 1：将 ORDER BY 改为 m_time (真实的定位时间)
                lagInFrame(MAINSTATIONID)
                    OVER (
                        PARTITION BY CARDID  -- 建议优先用全局唯一的硬件卡号分组
                        ORDER BY m_time ASC, u_time ASC
                    ) AS prev_station,
            ...
            for row in result.result_rows:
                ...
                try:
                    segment = {
                        ...
                        "进入时间": stats["earliest"],
                        "离开时间": stats["latest"],
                        "停留时长": stats["duration_seconds"],
                        "位置变化(米)": [str(d) for d in list(row[9])],
                        "记录数量": int(row[7])
                    }
