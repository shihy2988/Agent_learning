    ...
    def _register_prompts(self):
        ...
        def analysis_guide() -> str:
            ...
            - **关键参数技巧**：
              ...
              - **info_needs=True**：返回详细人员信息，包括人员基本信息以及其所在位置，以及当前井下人员区域分布。
              - **info_needs=False**：返回人员名单，包括人员基本信息以及其所在位置，以及当前井下人员区域分布。
    ...
    def _register_tools(self):
        ...
        def query_person_underground_status(
            ...
            - 获取今日矿下人员数据,包含人员基本信息、职务、工种、所在区域、进入时间等关键字段（通过now_only=False参数控制）
              - 如果今日有人员班次改变也会返回
            - 如果info_needs为True，则返回人员基本信息和现有位置，如果为False，则返回人员名称,默认返回人员名称。
            ...
            try:
                ...
                for person in today_persons:
                    ...
                    info = f"{dyty_name};{work_type};{class_time};{department}".strip(",")
                    if name not in leave_person:
                        leave_person[name] = {
                            "基本信息": set()
                        }
                    # 添加，自动去重
                    leave_person[name]["基本信息"].add(info)
                 
                    if name in persons_info:
                        continue

                    status_text = "已出井"
                    info = f"{duty_name};{work_type};{class_time};{department};{status_text}".strip(",")
                    out_names[name] = info
                 
                # 统计
                total_underground = len(persons_info)
                ...
                if info_needs:
                    json_res = json.dumps(
                        {
                            "现有井下人员数量": total_underground,
                            "今日出井人员数量": total_out_today,
                            "今日总共出入井人员数量": total_underground+total_out_today,
                            "目前井下人员信息": persons_info,
                            "同一人不同班次入井":banci_status,
                            "出井人员信息": out_names,
                            "现有位置分布": dict(
                                sorted(
                                    area_stats.items(), key=lambda x: x[1], reverse=True
                    ...
                    json_res = json.dumps(
                        {
                            ...
                            "今日出井人员数量": total_out_today,
                            "今日总共出入井人员数量": total_underground+total_out_today,
                            "目前井下人员名称": list(persons_info.keys()),
                            "同一人不同班次入井":banci_status,
                            "出井人员名称": list(out_names.keys()),
                            "现有位置分布": dict(
                                sorted(
                                    area_stats.items(), key=lambda x: x[1], reverse=True
                                )
                ...
                return json.dumps(
                    {
                        ...
                        "error": "查询失败",
                        "stop": True,
                        "instruction": "查询执行异常，请停止后续查询，不要尝试其他数据源或扩大查询范围"
                    },
                    ensure_ascii=False,
                    separators=(",", ":"),
                )
            ...
            try:
                ...
                if not person_records or not person_records.get("cards"):
                    ...
                    return json.dumps({
                        "status": "empty",
                        "message": f"未找到姓名 '{name}' 在指定时间范围内的记录。",
                        "instruction": "查询结果为空，请停止后续查询，不要尝试其他数据源或扩大查询范围。"
                    }, ensure_ascii=False)
                ...
                for card in cards:
                    ...
                    card["入井轨迹"] = resnew
                    card["轨迹数量"] = len(resnew)
                    
                # 返回结构统一
                output = {
                    "轨迹记录": cards,
                    "desc": '回答结束不要尝试其他数据源或扩大查询范围。'
                }

                json_str = json.dumps(output, ensure_ascii=False, separators=(",", ":"))
                len_json_str = len(json_str)
                logger.info(f"首次检查到 返回 json长度{len_json_str}")
                if len_json_str > 20000:
                    # 返回精简内容
                    simple_cards = []
                    for card in cards:
                        simple_segments = []
                        for seg_s in card.get("入井轨迹", []):
                            prev_seg = {
                                "入井时间": seg_s.get("入井时间"),
                                "出井时间": seg_s.get("出井时间"),
                                "入井地点": seg_s.get("入井地点"),
                                "出井地点": seg_s.get("出井地点"),
                                "单次入井时长": seg_s.get("入井时长"),
                                "单次出入井轨迹": []
                            }
                            segment_list = []
                            for seg in seg_s.get("入井轨迹", []):
                                station = seg.get("主站位置", " ")
                                seg_start = seg.get("进入时间")
                                seg_end = seg.get("离开时间")
                                duration_time =  seg.get("停留时长")
                                classname = seg.get('班次')
                                segment_item = [classname, station, seg_start, seg_end,duration_time]
                                segment_list.append(segment_item)
                            prev_seg["单次出入井轨迹"] = segment_list
                            prev_seg["字段描述"] = ['班次', '井下位置', '开始时间', '结束时间','停留时长/s']
                            prev_seg["单次出入井轨迹数量"] = len(segment_list)
                            simple_segments.append(prev_seg)
                        slim_card = {
                            "卡号": card.get("cardId") or card.get("cardID"),
                            "姓名": card.get("name"),
                            "部门": card.get("department"),
                            "工种": card.get("workType"),
                            "职位": card.get("job"),
                            "入井次数": len(simple_segments),
                            "入井轨迹": simple_segments
                        }
                        simple_cards.append(slim_card)
                    slim_output = {
                        "轨迹记录": simple_cards,
                        "desc": '由于数据过多,精简回答,需要详细信息请给出更精准的限制信息,回答结束不要尝试其他数据源或扩大查询范围。'
                    }
                    ...
                    json_res = json.dumps(
                        {
                            ...
                            "今日总车辆数": total_underground+total_out_today,
                            "车辆信息": cars,
                            "当前车辆区域分布": dict(
                                sorted(area_stats.items(), key=lambda x: x[1], reverse=True)
                            ),
        ...
        def query_cars_list(
            ...
            try:
                ...
                if statistics_filter_passed:
                    ...
                    if len_json_full > 40000:
                        logger.info("json_full超过40k，对每日数据做首次筛选压缩")
                        new_outs_record = {'每日数据': {}}
                        day_datas = car_records.get('每日数据')
                        for key, day_data in day_datas.items():
...
async def test_all_tools():
    ...
    try:
        ...
        # print("\n2️⃣ query_person_trajectory 基础筛选（全部默认参数）")
        # result = await mcp_app.call_tool("query_person_trajectory", {"name":"石小龙","start_time": "2026-08-04 00:00:00","end_time": "2026-08-04 10:59:59",})
        # pprint(result)
        # #
        # numeric_filters = {
        #     "距离主站距离/m": {"op": "<", "value": 150},
        #     "入井时长(秒)": {"op": ">", "value": 9 * 3600},
        #     # 更多按需添加...
