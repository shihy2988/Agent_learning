...
def generate_statistics(filtered_persons, keep_keys=["all"], now=None):
    """
    统计指定人员数据。
    新增参数 now: 指定"当前"时间点，默认为系统当前时间，用于计算当前在井下人数。
    """
    if now is None:
        now = datetime.now()
    ...
    stats = {
        ...
        "工种分布/人": Counter(),
        "班次分布/人": Counter(),
        "每小时人数统计/人": defaultdict(set),
    }

    person_set = set()
    current_underground_count = 0  # === 新增：实时计数变量 ===
    ...
    for person_key, person in filtered_persons.items():
        ...
        # ---------- 部门/职位/工种/班次 ----------
        department = person.get("部门")
        post = person.get("职位")
        worktype = person.get("工种")
        classtype = person.get("班次")
        ...
        if classtype:
            stats["班次分布/人"][classtype] += 1

        records = person.get("出入井记录", [])
        ...
        if records:
            ...
            latest = records[-1]
            in_time = parse_time(latest.get("入井时间"))
            out_time = parse_time(latest.get("出井时间"))
            if (in_time and in_time <= datetime.now()) or in_time is None:
                if out_time is None or out_time >= datetime.now():
                    current_underground_count += 1
        ...
        for segment in records:
            ...
            stats["入井地点分布/人次"][segment.get("入井地点")] += 1
            if not segment.get("出井地点"):
                station = '井下未出井'
            else:
                station = segment.get("出井地点")
            stats["出井地点分布/人次"][station] += 1
            ...
            for record in segment.get("具体轨迹变化", []):
                ...
                sub = record.get("分站名称")
                record_in_time = parse_time(record.get("进入时间")) or in_time
                ...
                if record_in_time:
                    ...
                    next_hour = (hour + 1) % 24
                    period = f"{record_in_time.strftime('%Y-%m-%d')} {hour:02d}-{next_hour:02d}点"
                ...
                if main:
                    stats["主站分布/条"][main].add(name)
                if sub:
                    stats["分站分布/条"][sub].add(name)
    ...
    # ========== 写入当前在井下人数 ==========
    stats["当前在井下人数"] = current_underground_count
    ...
    stats["入井地点分布/人次"] = format_with_unit(stats["入井地点分布/人次"])
    stats["出井地点分布/人次"] = format_with_unit(stats["出井地点分布/人次"])
    ...
    stats["班次分布/人"] = format_with_renming(stats["班次分布/人"])
