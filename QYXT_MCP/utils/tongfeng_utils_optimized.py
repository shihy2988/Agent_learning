...
class TongfengService:
    ...
    # =========================================================
    # 智能原子化缓存机制：新增的两个代理函数   "一号风机系统", "二号风机系统","变频器系统","高压柜系统","阀门系统","进线柜系统","风门系统","母联柜系统","系统级","设备级"]
    # =========================================================
    # INSERT_YOUR_CODE
    ...
    def _run_daily_auto_analysis(self):
        """
        每天定时自动分析前一月全量数据，并用 print_tongfeng_today_with_cache 和 calc_gonglv_energy_with_cache 预热缓存。
        【新增】每次执行完毕后自动清理60天以上的过期缓存数据。
        """
        systems = [
            "一号风机系统", "二号风机系统", "变频器系统", "高压柜系统",
            "阀门系统", "进线柜系统", "风门系统", "母联柜系统", "系统级", "设备级"
        ]

        db_path = os.path.join(os.path.dirname(__file__), "tongfeng_analysis_cache.db")
        ...
        while True:
            ...
            try:
                ...
                for sys in systems:
                    try:
                        self.print_tongfeng_today_with_cache(
                            system_name_filters=[sys],
                            start_date=start_date,
                            end_date=end_date,
                            enable_stable_periods=True
                        )
                        self.logger.info(f"自动分析: 系统 {sys} print_tongfeng_today_with_cache 完成")
                    except Exception as e:
                        self.logger.error(f"自动分析: 系统 {sys} print_tongfeng_today_with_cache 异常: {e}")
                ...
                try:
                    ...
                    # 清理通风系统原子缓存
                    cursor.execute(
                        "DELETE FROM tongfeng_atom_cache WHERE day_or_interval < ?",
                        (cutoff_date,)
                    )
                    ...
                    self.logger.info(
                        f"缓存清理完成: tongfeng_atom_cache 删除 {tf_deleted} 条, "
    ...
    def print_tongfeng_today_with_cache(
            self,
            system_name_filters: Union[List[str], str],
            start_date: Union[str, datetime, None] = None,
            end_date: Union[str, datetime, None] = None,
        ...
        【更新】最近7天（含当天）的数据强制走数据库实时查询，不使用缓存。
        """
        db_path = os.path.join(os.path.dirname(__file__), "tongfeng_analysis_cache.db")
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS tongfeng_atom_cache (
                sys_name TEXT,
                sg_name TEXT,
                cn_desc TEXT,
        ...
        # 处理 end_date 字符串时间部分为 00:00:00 的特殊情况
            ...
            for sg_name, metas in fan_system[sys_name].items():
                ...
                for en_key, cn_desc in metas.items():
                    ...
                    for day in req_dates:
                        ...
                        # 超过7天的历史数据才查缓存
                        cursor.execute('''
                            SELECT result_json FROM tongfeng_atom_cache 
        ...
        if missed_tasks:
            ...
            self.logger.info(f"【SQLite 缓存局部未命中/近7天实时查询】系统: {list(missed_systems)}, 子组: {list(missed_subgroups)}, 触发 ClickHouse 增量式分析...")
            ...
            for start_day, end_day in missed_ranges:
                ...
                part_data = self.print_tongfeng_today_by_yaml_system(
                ...
                # 按天入库（注意：近7天的数据也会写入缓存，但下次查询仍会因在7天窗口内而被跳过）
                        ...
                        for c_desc, local_out in g_val.items():
                            ...
                            if isinstance(local_out, dict) and "每日数据" in local_out:
                                for day_str, day_analysis in local_out["每日数据"].items():
                                    cursor.execute('''
                                        INSERT OR REPLACE INTO tongfeng_atom_cache 
        ...
        for sys_name in valid_systems:
            for sg_name, metas in fan_system[sys_name].items():
                ...
                for en_key, cn_desc in metas.items():
                    ...
                    v_filter_str = str(v_filter)
                    for day in req_dates:
                        # 所有日期统一从缓存读取（近7天的数据在上一步已实时查询并写入缓存）
                        cursor.execute('''
                            SELECT result_json FROM tongfeng_atom_cache 
                            WHERE sys_name=? AND sg_name=? AND cn_desc=? AND day_or_interval=? 
                            AND value_filter_str=? AND enable_stable_periods=?
    ...
    def calc_gonglv_energy_with_cache(self, start_date=None, end_date=None) -> Dict:
        ...
        # 如果没有需要增量拉取的天
        if not missing_days:
            self.logger.info("【SQLite 每日能耗缓存全命中（近7天除外）】直接重组内存数据。")
            conn.close()
            return hit_data

        self.logger.info(f"【SQLite 每日能耗缓存部分未命中/近7天实时查询】缺失天: {missing_days}，开始增量拉取...")
        ...
        # 写入缓存（近7天的数据也会写入，但下次查询仍会因在7天窗口内而被跳过）
        ...
        # 汇总：所有 requested_days 统一从缓存读取
        # （近7天的数据在上一步已实时查询并写入缓存，此处可一并读出）
        for day in requested_days:
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    res1 = service.print_tongfeng_today_with_cache(
    ...
    with open("通风_data.txt", "w", encoding="utf-8") as f:
        f.write(daytype_json)
        print("全部数据已成功写入 通风_data.txt")
    # print(daytype_json)
    ...
    with open("通风energy_data.txt", "w", encoding="utf-8") as f:
        f.write(daytype_json)
        print("全部数据已成功写入 通风energy_data.txt")
