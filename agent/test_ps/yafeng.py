...
print(f"压风.yaml 字段数量: {len(yaml_keys)}")

# 检查 yaml key 是否都在 columns 中
notfound = [k for k in yaml_keys if k not in columns]
if not notfound:
    print("压风.yaml 中所有 key 都存在于 ClickHouse 字段中。")
else:
...
with open("yafeng_system.yaml", "r", encoding="utf-8") as f:
    yafeng_yaml = yaml.safe_load(f)

level4_keys = get_level4_keys(yafeng_yaml)
print(f"yafeng_system.yaml 第4级（最内层）key 数量: {len(level4_keys)}")

# 查询并打印 1/2/3号空压机的状态
# 关联yafeng_system.yaml (29-32):
#   YF_YI_HAO_YI_WEI_WEI_BEI_YONG: 1 号空压机已处于备用状态
#   YF_YI_HAO_YI_TOU_RU: 1 号空压机已投入运行状态
#   YF_YI_HAO_YI_BEI_YONG: 1 号空压机已设置为备用状态
#   YF_YI_HAO_YI_JIAN_XIU: 1 号空压机已设置为检修状态
status_keys = [
    # 1号
    "YF_YI_HAO_YI_WEI_WEI_BEI_YONG", "YF_YI_HAO_YI_TOU_RU", "YF_YI_HAO_YI_BEI_YONG", "YF_YI_HAO_YI_JIAN_XIU",
    # 2号
    "YF_ER_HAO_YI_WEI_WEI_BEI_YONG", "YF_ER_HAO_YI_TOU_RU", "YF_ER_HAO_YI_BEI_YONG", "YF_ER_HAO_YI_JIAN_XIU",
    # 3号
    "YF_SAN_HAO_YI_WEI_WEI_BEI_YONG", "YF_SAN_HAO_YI_TOU_RU", "YF_SAN_HAO_YI_BEI_YONG", "YF_SAN_HAO_YI_JIAN_XIU",
]

status_sql = f"""
SELECT {', '.join(status_keys)}, {time_name}
FROM {database}.{table}
ORDER BY {time_name} DESC
LIMIT 20
"""

status_rows = client.query(status_sql).result_rows
print("最近20条 1/2/3号空压机状态数据（每台4个字段，每行一组时间）：")
print(status_keys + [time_name])
...
if not notfound_yafeng:
    print("yafeng_system.yaml 所有第4级字段都存在于 ClickHouse 字段中。")
else:
    print(f"以下yafeng_system.yaml的第4级key未出现在 ClickHouse 字段中: {notfound_yafeng}")

if not notfound_in_yaml:
    print("所有 ClickHouse 字段都能在 yafeng_system.yaml 第4级字段中找到。")
else:
    print(f"以下 ClickHouse 字段在 yafeng_system.yaml 第4级字段中没有找到: {notfound_in_yaml}")
